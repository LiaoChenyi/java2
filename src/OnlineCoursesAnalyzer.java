...
public class OnlineCoursesAnalyzer {
    ...
    public Map<String, Integer> getPtcpCountByInstAndSubject() {
        return courses.stream().collect(Collectors.groupingBy
            (s->s.getInstitution()+"-"+s.getSubject(),Collectors.summingInt(Course::getParticipants)))
    ...
    public Map<String, List<List<String>>> getCourseListOfInstructor() {
        ...
        for (int i=0;i<courses.size();i++) {
            instructorsList = courses.get(i).splitInstructors(instructorsList);
        }
        return instructorsList.stream().collect(Collectors.toMap(Instructor::getName,Instructor::getBothCourse));
    }
    ...
    public List<String> searchCourses(String courseSubject, double percentAudited, double totalCourseHours) {
        return courses.stream().filter(s->(s.getSubject().toLowerCase().contains(courseSubject.toLowerCase())
    ...
    public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
        ...
        return courses1.stream().collect(Collectors.toMap(Function.identity(),s->
            Math.pow(age-s.getAverageAge(),2)+Math.pow(gender*100-s.getAverageMale(),2)
                +Math.pow(isBachelorOrHigher*100-s.getAverageDegree(),2))).entrySet().stream().sorted((o1, o2) -> {
            if (Objects.equals(o1.getValue(), o2.getValue())&&o1.getKey().getNumber().equals(o2.getKey().getNumber())){
        ...
        }).filter(a->map.put(a.getKey().getNumber(),a.getValue())==null&&map.put(a.getKey().getTitle(),a.getValue())==null)
        ...
        for (String instructor : instructors) {
            if (instructorList.stream().noneMatch(s->s.getName().equals(instructor))){
                ...
                instructorList=new ArrayList<>(instructorList.stream().map(s-> {
                    ...
                    if (!s.coCourse.contains(this)&&instructors.size()>1)  s.setCoCourse(this);
                    return s;
                }).toList());
            }
        }
...
class Instructor {
    ...
    public List<List<String>> getBothCourse() {
        return Stream.of(getSinCourse().stream().sorted().toList(),getCoCourse().stream().sorted().toList()).toList();
    }
}